# -*- coding: utf-8 -*-
"""3-2.(실습)img_전이학습_cats vs dogs_

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1dJRk3tjC7_l0Pe73uOzo8WaQrZgbvyaf
...
""" + imagenet: 이미지 분류기에 대한 가중치를 가져온다는 의미 
 + weights= None : 학습된 가중치는 빼고 구조만가져온다 
 + input_shape=(224,224,3) : 224 by 224 크기의 컬러사진 
 + include_top =False로 하지 않으면 VGG16이 학습한 1000개 fc레이어까지 모두 가져옴 => 성능이 오히려 떨어진다. 
 + trainable = False : 기존 VGG16의 13개 레이어의 가중치를 가져오는데(얼린다고 표현) freeze 하지 않으면 가중치들이 내가 학습시키는 데이터에 맞게 업데이트를 시킴 => vgg16 쓰는 의미 없어짐. 그래서 반드시 trainable = False 시켜준다. 
"""
...
"""### 결과 해석
+ Trainable params: 12,911,490 => 내가 학습시키는 파라미터
+ Non-trainable params: 14,714,688 =>  vgg16 모델에서 얼려서 그대로 가용하는 파라미터
