# -*- coding: utf-8 -*-
"""2-1.(실습)structured_DNN_iris

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ySCTuNkIDlUi3q-xcjmydB1omd9sQQN8
...
# # train_dataset 하나씩 찍어보기(5개만)
# for data in train_dataset.take(5):
#   x = data['features']  # 2. feature = x, label = y 분할하기 
#   y = data['label']

#   y = tf.one_hot(y,3) # 1. label값을 one-hot encoding 하기. y가 3인 이유: 데이터셋 문서에 FeaturesDict -> num_classes 에 나와있다. 
#   print(x)
#   print(y)
...
가중치 W 들이 가장 근접한 Y값을 찾아가면서 업데이트를 계속하는데, 이 때 1epoch마다 업데이트를 하는게 아니라 1batch 마다 업데이트 진행.
...
+ batch_size가 작다 <br/>
  ...
  = 업데이트를 자주 한다 <br/>
  = 정교하게 결과에 도달 <br/>
  = 학습 속도가 느림, 시작 학습방향이 잘못된 경우 오히려 결과와 오차가 벌어질 수 있다.

+ batch_size가 크다 <br/>
  = batch 갯수가 적다 <br/>
  = 속도가 빠르다 <br/>
  = batch_size가 커서 학습시 out of memory(oom) 오류가 날 수 있다. <br/>
  = 정교하게 결과에 도달하기 어렵다.
...
train_data = train_dataset.map(preprocess).batch(batch_size) # train_data와 train_dataset의 변수 이름 중복되지않게 설정!
...
model = tf.keras.models.Sequential([
    ...
    Dense(512, activation='relu', input_shape=(4,)),  # 정형데이터에는 Flatten 필요없다.
...
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc']) # one_hot encoding 했으므로 loss='categorical_crossentropy'
...
history = model.fit(train_data,  # 전처리 맵핑한 train_data
                    validation_data=(valid_data), # 전처리 맵핑한 valid_data
                    epochs=20,
